	...
	while (1)
	{
		...
		switch (opcode)
		{
			...
			case DC::kLookupPointer:
			{
				...
				printf("DC::kLookupInteger/kLookupFloat: R%d R%d -> hashid: 0x%016llX(%s)\n", dstReg, operand1, pSymbolTbl[operand1].m_U64, StringIdToStringInternal(pSymbolTbl[operand1].m_U32));
			...
			*/
			...
			case DC::kCall:
			{
				...
				if (pValue)
				{
					...
					printf("DC::kCall: '%s(", StringIdToStringInternal(g_stackFrame->m_registers[operand1].m_U32));
			...
			case DC::kCallFf:
			{
				...
				printf("DC::kCallFf: '%s(", StringIdToStringInternal(g_stackFrame->m_registers[operand1].m_U32));
			...
			case DC::kLoadStaticU64Imm:
			{
				...
				printf("DC::kLoadStaticPointerImm/kLoadStaticI64Imm/kLoadStaticU64Imm: R%d, %.16llX(%s)\n", dstReg, pSymbolTbl[operand1].m_U64, StringIdToStringInternal(pSymbolTbl[operand1].m_U32));
				idx = tmpIdx;
				break;
			}
...
void PrintCallArgs(DC::StackFrame* pFrame, int count)
{
	if (count > 0)
	{
		...
		{
			printf("'%s ", StringIdToStringInternal(pFrame->m_registers[regIdx].m_U32));
			regIdx++;
		}
	}
}